#ifndef MULTI_RES_LAZY_THETA_STAR_H_
#define MULTI_RES_LAZY_THETA_STAR_H_
...
namespace wavemap {
/**
 * Config struct for Multi-Resolution Lazy Theta* planner.
 */
struct MultiResLazyThetaStarConfig
    : ConfigBase<MultiResLazyThetaStarConfig, 2> {
  ...
  // Constructors
  MultiResLazyThetaStarConfig() = default;
  MultiResLazyThetaStarConfig(FloatingPoint acceptable_relative_difference,
                              IndexElement initialization_height)
      : refinement_epsilon(acceptable_relative_difference),
        initialization_min_height(initialization_height) {}

  bool isValid(bool verbose) const override;
};

class MultiResLazyThetaStar : public PlannerBase {
 public:
  MultiResLazyThetaStar(const MultiResLazyThetaStarConfig& config,
 ...
 private:
  ...
  const MultiResLazyThetaStarConfig config_;
  ...
  const FloatingPoint half_min_cell_width_ = 0.49f * min_cell_width_;
  ...
  bool checkNode(const OctreeIndex& node_index,
                 MultiResCostField::Node::DataType& node);
  ...
  void pull(const OctreeIndex& sink_index,
            MultiResCostField::Node::DataType& sink_node);
  void recursivePull(const OctreeIndex& sink_index,
                     MultiResCostField::Node::DataType& sink_node,
                     FloatingPoint& sink_g_score,
                     const OctreeIndex& neighbor_index,
  ...
  enum class UpdateAction { kOld, kSourcePredecessor, kRefine };
...
#endif  // MULTI_RES_LAZY_THETA_STAR_H_
