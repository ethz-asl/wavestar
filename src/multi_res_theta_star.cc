...
namespace wavemap {
DECLARE_CONFIG_MEMBERS(MultiResThetaStarConfig,
                      (refinement_epsilon)
                      (initialization_min_height));

bool MultiResThetaStarConfig::isValid(bool verbose) const {
  bool is_valid = true;
...
std::vector<Point3D> MultiResThetaStar::searchPath(const Point3D& start,
                                                   const Point3D& goal) {
  ProfilerZoneScoped;
  if (solve(start, goal)) {
    return getPath();
...
bool MultiResThetaStar::solve(const Point3D& start, const Point3D& goal) {
  ProfilerZoneScoped;
  ...
  auto& start_node = cost_field_.getOrAllocateNode(start_index_);
...
std::vector<Point3D> MultiResThetaStar::getPath() const {
  ProfilerZoneScoped;
...
void MultiResThetaStar::seedBlock(const OctreeIndex& block_index) {
  ProfilerZoneScoped;
...
void MultiResThetaStar::recursiveSeedBlock(  // NOLINT
...
bool MultiResThetaStar::propagate(const OctreeIndex& goal_index,
                                  const Point3D& goal) {
  ProfilerZoneScoped;
...
void MultiResThetaStar::recursivePush(  // NOLINT
    const OctreeIndex& source_index,
  ...
  if (min_height_reached || neighbor_is_free_leaf) {
    FloatingPoint source_g_score = std::numeric_limits<FloatingPoint>::max();
    const auto action = determineUpdateAction(
        source_index, source_g_score, source_node, neighbor_index,
        neighbor_node.data(), min_height_reached);
    ...
    if (action == UpdateAction::kSourceDirect) {
      neighbor_node.data().predecessor_index = source_index;
      neighbor_node.data().predecessor_g_score = source_g_score;
      addToOpenQueue(neighbor_index, neighbor_node, goal);
      return;
    }
  ...
  for (NdtreeIndexRelativeChild relative_child_idx = 0;
    ...
    if (!child_node) {
      ...
      if (!cost_field_raw_.equalsDefaultValue(neighbor_node.data()) &&
          hasLineOfSight(child_index, neighbor_node.data().predecessor_index)) {
        child_node->data() = neighbor_node.data();
...
void MultiResThetaStar::addToOpenQueue(const OctreeIndex& node_index,
  ...
  const FloatingPoint f_score = computeFScore(
      ...
      node_index, goal, min_cell_width_, half_cell_width_at_min_height_);
  open_queue_.emplace(FScoreAndMultiResIndex{f_score, node_index, &node});
}

bool MultiResThetaStar::hasLineOfSight(const OctreeIndex& node_index,
...
MultiResThetaStar::UpdateAction MultiResThetaStar::determineUpdateAction(
    ...
    bool cannot_refine) {
  const bool source_predecessor_visible =
      hasLineOfSight(neighbor_index, source_node.predecessor_index);
  const UpdateAction visible_source = source_predecessor_visible
                                          ? UpdateAction::kSourcePredecessor
                                          : UpdateAction::kSourceDirect;

  if (source_predecessor_visible &&
      neighbor_node.predecessor_index == source_node.predecessor_index) {
  ...
  const auto source_center =
      convert::nodeIndexToCenterPoint(source_index, min_cell_width_);
  const auto neighbor_center =
      convert::nodeIndexToCenterPoint(neighbor_index, min_cell_width_);

  const FloatingPoint d_source_predecessor_source =
      (source_center - source_predecessor_center).norm();
  source_g_score =
      source_node.predecessor_g_score + d_source_predecessor_source;

  if (cannot_refine) {
    const FloatingPoint d_current_predecessor_neighbor =
        (neighbor_center - current_predecessor_center).norm();
    const FloatingPoint neighbor_g_score_current =
        neighbor_node.predecessor_g_score + d_current_predecessor_neighbor;

    FloatingPoint neighbor_g_score_source;
    if (source_predecessor_visible) {
      const FloatingPoint d_source_predecessor_neighbor =
          (neighbor_center - source_predecessor_center).norm();
      neighbor_g_score_source =
          source_node.predecessor_g_score + d_source_predecessor_neighbor;
    } else {
      const FloatingPoint d_source_neighbor =
          (neighbor_center - source_center).norm();
      neighbor_g_score_source = source_g_score + d_source_neighbor;
    }

    if (neighbor_g_score_current < neighbor_g_score_source + kEpsilon) {
      return UpdateAction::kOld;
    }
  ...
  neighbor_aabb.min.array() += half_cell_width_at_min_height_;
  neighbor_aabb.max.array() -= half_cell_width_at_min_height_;

  constexpr int kNumAabbCorners = AABB<Point3D>::kNumCorners;
  Eigen::Array<FloatingPoint, kNumAabbCorners, 1> d_current_corner;
  Eigen::Array<FloatingPoint, kNumAabbCorners, 1> d_source_corner;
  Eigen::Array<FloatingPoint, kNumAabbCorners, 1> g_scores_current;
  Eigen::Array<FloatingPoint, kNumAabbCorners, 1> g_scores_source;
  if (source_predecessor_visible) {
    for (int corner_idx = 0; corner_idx < kNumAabbCorners; ++corner_idx) {
      const Point3D corner = neighbor_aabb.corner_point(corner_idx);
      d_current_corner[corner_idx] =
          (corner - current_predecessor_center).norm();
      d_source_corner[corner_idx] = (corner - source_predecessor_center).norm();
    }
    g_scores_current = neighbor_node.predecessor_g_score + d_current_corner;
    g_scores_source = source_node.predecessor_g_score + d_source_corner;
  } else {
    for (int corner_idx = 0; corner_idx < kNumAabbCorners; ++corner_idx) {
      const Point3D corner = neighbor_aabb.corner_point(corner_idx);
      d_current_corner[corner_idx] =
          (corner - current_predecessor_center).norm();
      d_source_corner[corner_idx] = (corner - source_center).norm();
    }
    g_scores_current = neighbor_node.predecessor_g_score + d_current_corner;
    g_scores_source = source_g_score + d_source_corner;
  }

  if ((g_scores_current < g_scores_source + kEpsilon).all()) {
    return UpdateAction::kOld;
  }
  if ((g_scores_source < g_scores_current + kEpsilon).all()) {
    return visible_source;
  }

  const bool current_acceptable =
      (g_scores_current - g_scores_source <
       d_current_corner * config_.refinement_epsilon + kAbsolutePrecision)
          .all();
  if (current_acceptable) {
    return UpdateAction::kOld;
  }

  const bool source_acceptable =
      (g_scores_source - g_scores_current <
       d_source_corner * config_.refinement_epsilon + kAbsolutePrecision)
          .all();
