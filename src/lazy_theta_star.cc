...
std::vector<Point3D> LazyThetaStar::searchPath(const Point3D& start,
                                               const Point3D& goal) {
  ProfilerZoneScoped;
  if (solve(start, goal)) {
    return getPath();
...
bool LazyThetaStar::solve(const Point3D& start, const Point3D& goal) {
  ProfilerZoneScoped;
  ...
  while (!open_queue_.empty()) {
    ...
    const Ray ray(cell_index, cell.predecessor);
    ...
    if (!predecessor_visible) {
      constexpr auto kMaxGScore = std::numeric_limits<FloatingPoint>::max();
      cell.g_score = kMaxGScore;
      for (size_t neighbor_idx = 0; neighbor_idx < kNeighborIndexOffsets.size();
        ...
        const auto& neighbor = cost_field_.getOrAllocateValue(neighbor_index);
        if (neighbor.closed) {
          const FloatingPoint candidate_g_score =
              neighbor.g_score + neighbor_distance_offsets_[neighbor_idx];
          if (candidate_g_score < cell.g_score) {
            cell.g_score = candidate_g_score;
            cell.predecessor = neighbor_index;
          }
        }
      }
      CHECK_NE(cell.g_score, kMaxGScore);
    }
    ...
    const auto& predecessor = cost_field_.getOrAllocateValue(cell.predecessor);
    for (const auto& index_offset : kNeighborIndexOffsets) {
      const Index3D neighbor_index = cell_index + index_offset;
...
std::vector<Point3D> LazyThetaStar::getPath() const {
  ProfilerZoneScoped;
